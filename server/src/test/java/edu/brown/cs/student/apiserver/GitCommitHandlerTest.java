...
public class GitCommitHandlerTest extends BaseEndpointTest {
  ...
  public void testMissingSessionId() {
    try {
      HttpURLConnection connection = tryRequest("gitcommit?user_id=" + userId + "&branch_id=" + branchId + "&commit_message=TestCommit");
  ...
  public void testMissingUserId() {
    try {
      HttpURLConnection connection = tryRequest("gitcommit?session_id=" + sessionId + "&branch_id=" + branchId + "&commit_message=TestCommit");
      Map<String, Object> response = deserializeResponse(connection);
      assertEquals("error_bad_request", response.get("response"));
      assertEquals("null parameter", response.get("error_cause"));
    } catch (Exception e) {
  ...
  public void testMissingBranchId() {
    try {
      HttpURLConnection connection = tryRequest("gitcommit?session_id=" + sessionId + "&user_id=" + userId + "&commit_message=TestCommit");
      Map<String, Object> response = deserializeResponse(connection);
      assertEquals("error_bad_request", response.get("response"));
      assertEquals("null parameter", response.get("error_cause"));
    } catch (Exception e) {
  ...
  public void testMissingCommitMessage() {
    try {
      HttpURLConnection connection = tryRequest("gitcommit?session_id=" + sessionId + "&user_id=" + userId + "&branch_id=" + branchId);
      Map<String, Object> response = deserializeResponse(connection);
      assertEquals("error_bad_request", response.get("response"));
      assertEquals("null parameter", response.get("error_cause"));
    } catch (Exception e) {
  ...
  public void testMismatchedParentCommitIds() {
    try {
      HttpURLConnection connection = tryRequest("gitcommit?session_id=" + sessionId + "&user_id=" + userId + "&branch_id=" + branchId + "&commit_message=TestCommit&local_commit_id=id1");
      Map<String, Object> response = deserializeResponse(connection);
      assertEquals("error_bad_request", response.get("response"));
    } catch (Exception e) {
      fail(e);
  ...
  public void testCommitNoChanges() {
    try {
      Map<String, List<MockFileObject>> fileMap = Map.of(
          "file1", List.of(new MockFileObject("data", "data"))
      );
      ...
      connection = tryRequest("gitcommit?session_id=" + sessionId + "&user_id=" + userId + "&branch_id=" + branchId + "&commit_message=Initialcommit");
  ...
  public void testSuccessfulCommit() {
    try {
      Map<String, List<MockFileObject>> fileMap = Map.of(
          "file1", List.of(new MockFileObject("data1", "data1"))
      );
      String fileMapJson = serializeFileMap(fileMap);
      ...
      Map<String, List<MockFileObject>> modifiedMap = Map.of(
          "file1", List.of(new MockFileObject("data2", "data2"))
      );
      String modifiedJson = serializeFileMap(modifiedMap);

      connection = tryRequest("gitadd?session_id=" + sessionId + "&user_id=" + userId + "&branch_id=" + branchId + "&file_map_json=" + modifiedJson);
      Map<String, Object> addResponse = deserializeResponse(connection);
      assertEquals("success", addResponse.get("response"));

      connection = tryRequest("gitcommit?session_id=" + sessionId + "&user_id=" + userId + "&branch_id=" + branchId + "&commit_message=Updatefile");
      Map<String, Object> commitResponse = deserializeResponse(connection);
