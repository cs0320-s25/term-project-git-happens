...
import edu.brown.cs.student.apiserver.BaseEndpointTest;
...
public class GitMergeHandlerTest extends BaseEndpointTest {
  ...
  public void testMissingParameters() {
    try {
      HttpURLConnection connection = tryRequest("gitmerge?user_id=" + userId + "&current_branch_id=" + currentBranch + "&merge_branch_id=" + mergeBranch + "&file_map_json={}");
  ...
  public void testSameBranchMerge() {
    try {
      String fileMapJson = serializeFileMap(Map.of("file1", List.of(new MockFileObject("$1", "$1"))));
      ...
      connection = tryRequest("gitmerge?session_id=" + sessionId + "&user_id=" + userId + "&current_branch_id=" + currentBranch + "&merge_branch_id=" + currentBranch + "&file_map_json=" + fileMapJson);
      Map<String, Object> response = deserializeResponse(connection);
  ...
  public void testNonexistentMergeBranch() {
    try {
      ...
      connection = tryRequest("gitmerge?session_id=" + sessionId + "&user_id=" + userId + "&current_branch_id=" + currentBranch + "&merge_branch_id=ghost&file_map_json=" + fileMapJson);
      Map<String, Object> response = deserializeResponse(connection);
  ...
  public void testUncommittedChangesBlockMerge() {
    try {
      ...
      String modifiedJson = serializeFileMap(Map.of("file1", List.of(new MockFileObject("$1", "$1"))));

      HttpURLConnection connection = tryRequest("createsession?session_id=" + sessionId + "&user_id=" + userId + "&file_map_json=" + originalJson);
      assertEquals("success", deserializeResponse(connection).get("response"));

      // add a new branch
      HttpURLConnection branchResponse = tryRequest("gitbranch?session_id=" + sessionId +
          "&user_id=" + userId +
          "&branch_request=" + mergeBranch +
          "&current_branch_id=" + currentBranch +
          "&file_map_json=" + originalJson);
      ...
      connection = tryRequest("gitmerge?session_id=" + sessionId + "&user_id=" + userId + "&current_branch_id=" + currentBranch + "&merge_branch_id=" + mergeBranch + "&file_map_json=" + modifiedJson);
      Map<String, Object> response = deserializeResponse(connection);
      assertEquals("error_database", response.get("response"));
  ...
  public void testSuccessfulAutoMerge() {
    try {
      ...
      HttpURLConnection connection = tryRequest("createsession?session_id=" + sessionId + "&user_id=" + userId + "&file_map_json=" + baseJson);
      assertEquals("success", deserializeResponse(connection).get("response"));
      ...
      HttpURLConnection branchResponse = tryRequest("gitbranch?session_id=" + sessionId +
          ...
          "&branch_request=" + mergeBranch +
          "&current_branch_id=" + currentBranch +
          "&file_map_json=" + baseJson);
      Map<String, Object> branchJson = deserializeResponse(branchResponse);
      assertEquals("success", branchJson.get("response"));

      // Modify merge branch and commit
      String mergeModified = serializeFileMap(Map.of(
          "file1", List.of(new MockFileObject("base", "base"), new MockFileObject("new", "new"))
      ));
      tryRequest("gitadd?session_id=" + sessionId + "&user_id=" + userId + "&branch_id=" + mergeBranch + "&file_map_json=" + mergeModified);
      tryRequest("gitcommit?session_id=" + sessionId + "&user_id=" + userId + "&branch_id=" + mergeBranch + "&commit_message=extend");

      // Merge into base
      connection = tryRequest("gitmerge?session_id=" + sessionId + "&user_id=" + userId + "&current_branch_id=" + currentBranch + "&merge_branch_id=" + mergeBranch + "&file_map_json=" + baseJson);
      Map<String, Object> response = deserializeResponse(connection);
      assertEquals("success", response.get("response"));
