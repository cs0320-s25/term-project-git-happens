...
public class GitCheckoutHandlerTest extends BaseEndpointTest {
  ...
  public void testMissingParameters() {
    try {
      HttpURLConnection connection = tryRequest("gitcheckout?user_id=" + userId + "&current_branch_id=" + branchId + "&new_branch_id=" + newBranch + "&file_map_json={}");
  ...
  public void testDetectUncommittedChanges() {
    try {
      Map<String, List<MockFileObject>> fileMap = Map.of(
          "file1", List.of(new MockFileObject("original", "original"))
      );
      ...
      // Modify file map
      Map<String, List<MockFileObject>> modifiedFileMap = Map.of(
          "file1", List.of(new MockFileObject("changed", "changed"))
      );
      String modifiedJson = serializeFileMap(modifiedFileMap);

      connection = tryRequest("gitcheckout?session_id=" + sessionId + "&user_id=" + userId + "&current_branch_id=" + branchId + "&new_branch_id=" + branchId + "&file_map_json=" + modifiedJson);
      Map<String, Object> response = deserializeResponse(connection);
  ...
  public void testCheckoutCreatesLocalBranch() {
    try {
      Map<String, List<MockFileObject>> fileMap = Map.of(
          "file1", List.of(new MockFileObject("content", "content"))
      );
      String fileMapJson = serializeFileMap(fileMap);
      ...
      connection = tryRequest("gitbranch?session_id=" + sessionId + "&user_id=" + userId + "&branch_request=" + newBranch + "&current_branch_id=" + branchId + "&file_map_json=" + fileMapJson);
      assertEquals("success", deserializeResponse(connection).get("response"));
      // Now checkout the new branch
      connection = tryRequest("gitcheckout?session_id=" + sessionId + "&user_id=" + userId + "&current_branch_id=" + branchId + "&new_branch_id=" + newBranch + "&file_map_json=" + fileMapJson);
      Map<String, Object> response = deserializeResponse(connection);
  ...
  public void testCheckoutNonexistentBranch() {
    try {
      ...
      connection = tryRequest("gitcheckout?session_id=" + sessionId + "&user_id=" + userId + "&current_branch_id=" + branchId + "&new_branch_id=doesNotExist&file_map_json=" + fileMapJson);
      Map<String, Object> response = deserializeResponse(connection);

      assertEquals("error_database", response.get("response"));
      assertEquals("pathspec 'doesNotExist' did not match any branches known to git", response.get("error_cause"));
    } catch (Exception e) {
      fail(e);
    }
  }
